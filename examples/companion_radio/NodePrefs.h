...
public:
  ...
  // TEAM: adaptive forwarding control (set by app via CMD_SET_MAX_HOPS)
  uint8_t flood_max = 0;         // Max hops for packet forwarding (0=disabled, 1-127=max hops)

  // TEAM: autonomous tracker settings
  uint8_t  autonomous_enabled = 0;         // 0=disabled, 1=enabled
  uint8_t  autonomous_channel_hash = 0;    // reserved for channel-targeted autonomous telemetry
  uint16_t autonomous_interval_sec = 30;   // autonomous interval (seconds)
  uint16_t autonomous_min_distance_m = 0;  // min movement trigger in meters (0=disabled)
...
private:
  ...
  protected:
    void structure() override {
      ...
      def("f_max", _parent->flood_max);   // TEAM: adaptive forwarding hop limit
  ...
  // TEAM: autonomous tracker settings
  class AutonomousPrefs : public ConfigSerializer {
    NodePrefs* _parent;
  protected:
    void structure() override {
      def("en", _parent->autonomous_enabled);            // 0=disabled, 1=enabled
      def("chan", _parent->autonomous_channel_hash);
      def("int", _parent->autonomous_interval_sec);      // interval in seconds
      def("dist", _parent->autonomous_min_distance_m);   // min movement trigger in metres
    }
  public:
    AutonomousPrefs(NodePrefs* parent) : _parent(parent) { }
  };
  AutonomousPrefs autonomous;
...
protected:
  void structure() override {
    ...
    def("repeat", repeat);
    def("comp", companion);
    def("auto", autonomous);   // TEAM: autonomous tracker settings
  }
public:
  NodePrefs() : radio(this), gps(this), repeat(this), companion(this), autonomous(this) {
    node_name[0] = 0;
